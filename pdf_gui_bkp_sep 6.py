...
class PDFProcessorGUI:
    ...
    def _process_pdf_thread(self, input_path, output_path):
        try:
            ...
            for i, page in enumerate(doc):
                ...
                for idx2, (sku, label_text) in enumerate(sku_labels):
                    if sku == "TN0001":
                        ...
                        final_labels.append(label_text)
                # Sort final_labels by custom priority and group identical items
                def label_key(label):
                    ...
                    if l == 'OIL':
                        return (1, l)
                    elif l.startswith('OILX2'):
                        return (2, l)
                    elif l.startswith('OILX3'):
                        return (3, l)
                    elif l.startswith('OILX'):
                        return (4, f"OILX{l[4:]}")
                    elif l == 'POTLI':
                        return (5, l)
                        ...
                        return (7, l)
                    elif l.startswith('POTLIX'):
                        return (8, f"POTLIX{l[6:]}")
                    elif l == 'ROLLON':
                        return (9, l)
                    elif l.startswith('ROLLONX2'):
                        return (10, l)
                    elif l.startswith('ROLLONX'):
                        return (11, f"ROLLONX{l[7:]}")
                    else:
                        return (99, l)
                
                # Sort by both priority and the exact label to group identical items
                final_labels.sort(key=label_key)
                if final_labels:
            ...
            # Prepare final page order with proper grouping
            final_page_order = []
            ...
            # Group all marked pages by their labels
            single_product_pages = []  # Pages with only one product type
            mixed_product_pages = []   # Pages with multiple product types
            
            for i, label_text in remaining_marked:
                # Count how many different products are on this page
            ...
            # Group single product pages by their labels
            page_groups = {}
            for i, label_text in single_product_pages:
                normalized_label = label_text.replace('→ ', '').replace(' ', '').upper()
                if normalized_label not in page_groups:
                    page_groups[normalized_label] = []
            ...
            # Add grouped marked pages in the specified order
            marked_and_grouped = []
            
            # First add all single product pages in the specified order
            for group in group_order:
                for label in sorted(page_groups.keys()):
                    if label.startswith(group):
                        marked_and_grouped.extend(page_groups[label])
            
            # Add any remaining single product groups that weren't in the predefined order
            for label in sorted(page_groups.keys()):
                if not any(label.startswith(group) for group in group_order):
                    marked_and_grouped.extend(page_groups[label])
            
            # Add the grouped pairs
            ...
            # Finally, add all mixed product pages at the very end
            marked_and_grouped.extend(mixed_product_pages)
